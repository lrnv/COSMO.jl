...
function set_accelerator!(ws::COSMO.Workspace)
  ...
  if typeof(ws.accelerator) == EmptyAccelerator{Float64}
    ...
    elseif ws.settings.accelerator == :anderson1
      ws.accelerator = AndersonAccelerator{Float64}(ws.p.m + ws.p.n, is_type1 = true, mem = ws.settings.acc_mem)
    elseif ws.settings.accelerator == :anderson2
      ws.accelerator = AndersonAccelerator{Float64}(ws.p.m + ws.p.n, is_type1 = false, mem = ws.settings.acc_mem)
    else
